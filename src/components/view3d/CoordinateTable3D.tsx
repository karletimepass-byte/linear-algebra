...
interface CoordinateTable3DProps {
...
export const CoordinateTable3D: React.FC<CoordinateTable3DProps> = ({
  originalPoints,
  transformedPoints,
  selectedPointLabel,
  onSelectPoint,
        ...
        <table className="w-full text-left text-xs font-mono">
          <thead>
            <tr className="border-b border-slate-700/70 text-slate-400 uppercase text-[10px] tracking-wider">
              ...
              <th className="pb-2">Original (x, y, z)</th>
              <th className="pb-2">Transformed (x', y', z')</th>
              <th className="pb-2">Displacement (Δx, Δy, Δz)</th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  ...
                  <td className="py-2.5 text-sky-300">
                    ({formatNumber(pOrig.x)}, {formatNumber(pOrig.y)}, {formatNumber(pOrig.z)})
                  </td>

                  <td className="py-2.5 text-emerald-400 font-semibold">
                    ({formatNumber(pTrans.x)}, {formatNumber(pTrans.y)}, {formatNumber(pTrans.z)})
                  </td>

                  <td className="py-2.5 text-amber-300/90">
                    ({formatNumber(dx)}, {formatNumber(dy)}, {formatNumber(dz)})
                  </td>
                </tr>
              );
            })}
